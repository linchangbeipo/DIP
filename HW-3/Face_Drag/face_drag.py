...
with gr.Blocks() as app:
    ...
    with gr.Row():
        ...
        with gr.Row():
            ...
            with gr.Column(scale=3):
                ...
                with gr.Row():
                    with gr.Column(scale=1, min_width=10):
                        gr.Markdown(value='Auto_PS', show_label=False)
                    with gr.Column(scale=1, min_width=10):
                        face_slim_btn = gr.Button("瘦脸")
                    with gr.Column(scale=1, min_width=10):
                        smile_btn = gr.Button("微笑")
                    with gr.Column(scale=1,min_width=10):
                        big_eyes_btn = gr.Button("大眼")
    ...
    face_feature_p = face_alignment.FaceAlignment(face_alignment.LandmarksType.TWO_D, device='cuda' if torch.cuda.is_available() else 'cpu')

    def face_feature_get(image):
        ## 读取脸部特征点
        img = np.array(image)
        feature = face_feature_p.get_landmarks(img)
        if feature is None:
            return None
        return feature[0]
    
    def face_slim_points(feature):
        ## 返回脸部特征点的移动
        points = {}
        right_face_s = feature[4:8]
        left_face_s = feature[10:14]

        for i in range(4):
            right_x, right_y = right_face_s[i]
            left_x, left_y = left_face_s[i]

            points[2*i] = {'start': [int(right_x), int(right_y)], 'target': [int(right_x+20), int(right_y)]}
            points[2*i+1] = {'start': [int(left_x), int(left_y)], 'target': [int(left_x-20), int(left_y)]}

        return points
    
    def on_click_face_slim(global_state):

        image = global_state['images']['image_raw']
        feature = face_feature_get(image)
        if feature is None:
            return global_state, global_state['images']['image_show']
        
        points_vec = face_slim_points(feature)
        global_state['points'] = points_vec
        image_draw = update_image_draw(
            image,
            points_vec,
            global_state['mask'],
    ...
    def smile_points(feature):
        right_s = feature[48]
        left_s = feature[54]

        points = {}
        points[0] = {'start': [int(right_s[0]), int(right_s[1])], 'target': [int(right_s[0]), int(right_s[1]-15)]}
        points[1] = {'start': [int(left_s[0]), int(left_s[1])], 'target': [int(left_s[0]), int(left_s[1]-15)]}
        return points
    ...
    def on_click_smile(global_state):
        ...
        points_vec = smile_points(feature)
        global_state['points'] = points_vec
        image_draw = update_image_draw(
            image,
            points_vec,
    ...
    def big_eyes_points(feature):
        points = {}
        # 获取左右眼关键点
        left_eye = feature[36:42]
        right_eye = feature[42:48]
        
        idx = 0
        for eye in [left_eye, right_eye]:
            center_x = np.mean(eye[:, 0])
            center_y = np.mean(eye[:, 1])
            for point in eye:
                x, y = point
                points[idx] = {
                    'start': [int(x), int(y)],
                    'target': [int(center_x + (x - center_x) * 1.2), int(center_y + (y - center_y) * 1.4)]
                }
                idx += 1
    ...
    def on_click_big_eyes(global_state):
        ...
        points_vec = big_eyes_points(feature)
        global_state['points'] = points_vec
        image_draw = update_image_draw(
            image,
            points_vec,
