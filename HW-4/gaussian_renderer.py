...
class GaussianRenderer(nn.Module):
    ...
    def compute_projection(
        ...
        f_x = K[0, 0]
        f_y = K[1, 1]
        J_proj[:, 0, 0] = f_x / depths
        J_proj[:, 1, 1] = f_y / depths
        J_proj[:, 0, 2] = -f_x * cam_points[:, 0] / (depths ** 2)
        J_proj[:, 1, 2] = -f_y * cam_points[:, 1] / (depths ** 2)
        
        # Transform covariance to camera space
        ### FILL: Aplly world to camera rotation to the 3d covariance matrix
        covs_cam = torch.matmul(R, torch.matmul(covs3d, R.transpose(0, 1)))
    ...
    def compute_gaussian_values(
        ...
        gaussian = torch.exp(-0.5 * torch.einsum('nhwi, nij, nhwj->nhw', dx, torch.inverse(covs2D), dx))
    ...
    def forward(
        ...
        # 7. Compute weights
        ### FILL:
        alpha_0 = torch.zeros_like(alphas[0, :, :])
        alpha_res = 1 - torch.cat([alpha_0.unsqueeze(0), alphas], dim=0)
