...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask, box_fore, box_blend):
    """
    ...
    laplace_kernel = torch.tensor([[0, 1, 0], [1, -4, 1], [0, 1, 0]],dtype=torch.float32,device = foreground_img.device)
    laplace_kernel = laplace_kernel.unsqueeze(0).repeat(1,3,1,1)
    fore_laplace = F.conv2d(foreground_img, laplace_kernel, stride=1, padding=1)
    blend_laplace = F.conv2d(blended_img, laplace_kernel, stride=1, padding=1)
    fore_laplace_mask = fore_laplace * foreground_mask
    blend_laplace_mask = blend_laplace * background_mask
    loss += torch.sum(torch.abs(fore_laplace_mask[0,0,box_fore[0,0]:box_fore[1,0], box_fore[0,1]:box_fore[1,1]] - blend_laplace_mask[0,0,box_blend[0,0]:box_blend[1,0], box_blend[0,1]:box_blend[1,1]]))
    loss /= torch.sum(foreground_mask)
...
def blending(foreground_image_original, background_image_original, dx, dy, polygon_state):
    ...
    # Get bounding boxes for foreground and blended images
    min_fore_x = np.min(foreground_polygon_points[:,1])
    min_fore_y = np.min(foreground_polygon_points[:,0])
    max_fore_x = np.max(foreground_polygon_points[:,1])
    max_fore_y = np.max(foreground_polygon_points[:,0])
    min_blend_x = np.min(background_polygon_points[:,1])
    min_blend_y = np.min(background_polygon_points[:,0])
    max_blend_x = np.max(background_polygon_points[:,1])
    max_blend_y = np.max(background_polygon_points[:,0])
    box_fore = torch.tensor([[min_fore_x, min_fore_y], [max_fore_x, max_fore_y]]).detach()
    box_blend = torch.tensor([[min_blend_x, min_blend_y], [max_blend_x, max_blend_y]]).detach()
    ...
    for step in range(iter_num):
        ...
        loss = cal_laplacian_loss(fg_img_tensor, fg_mask_tensor, blended_img_for_loss, bg_mask_tensor, box_fore, box_blend)
